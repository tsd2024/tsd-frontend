...
export default function RoomPage({ params }: { params: { roomId: string } }) {
  ...
  const [userStoryVoted, setUserStoryVoted] = useState<boolean>(false);

  const cards = [1, 2, 3, 5, 8, 13, 21];

  const userStoriesMock: UserStory[] = [
    {
      user_story_id: "1",
    ...
    {
      user_story_id: "2",
      title: "As a user, I want to be able to create a new project",
      points: 8,
    },
    {
      user_story_id: "3",
      title: "As a user, I want to be able to add tasks to a project",
      points: 3,
    },
    {
      user_story_id: "4",
      title: "As a user, I want to be able to mark tasks as completed",
      points: 5,
    },
    {
      user_story_id: "5",
      title: "As a user, I want to be able to delete my account",
      points: 8,
    },
    {
      user_story_id: "6",
      title:
        "As a user, I want to be able to invite others to collaborate on a project",
      points: 13,
    },
    {
      user_story_id: "7",
      title: "As a user, I want to be able to set deadlines for tasks",
      points: 5,
    },
    {
      user_story_id: "8",
      title: "As a user, I want to be able to prioritize tasks",
      points: 3,
    },
  ];
    ...
    `${WEBSOCKET_PROTOCOL}://${BACKEND_URL}/api/v1/room`,
    {
      ...
      onMessage: (event) => {
        ...
        if (receiveData.action === ActionType.LOBBY_STATE) {
          ...
          //setUserStories(userStoriesMock);
                  ...
                  <div className="flex flex-row space-x-4 items-center">
                    ...
                    <div
                      className={`w-3 h-3 rounded-full ${
                        player.ready ? "bg-green-400" : "bg-yellow-300"
                      }`}
                    ></div>
          ...
          <div className="flex flex-row space-x-4 items-center mr-2">
            ...
            <Sheet>
              ...
              <SheetContent className="overflow-auto">
                ...
                <div className="flex flex-col space-y-4">
                  {userStoriesMock.map((userStory) => (
                    <Card key={userStory.user_story_id}>
                      <CardHeader>
                        <CardTitle>{userStory.user_story_id}</CardTitle>
                        <CardDescription>
          ...
          <div className="flex flex-row space-x-4 items-center justify-center">
            {cards.map((card) => (
              <Button
                variant={"outline"}
                key={card}
                className={`w-12 h-20 rounded-lg shadow-md transition-transform duration-300 transform hover:-translate-y-1.5 ${
                  selectedCard === card ? "bg-green-400 hover:bg-green-400" : ""
                }`}
